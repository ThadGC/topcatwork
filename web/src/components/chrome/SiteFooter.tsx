  ...
  return (
    <footer className="site" id="footer" ref={footer}>
      ...
      <div className="foot-bar">
        ...
        {/*
          THE STUDIO CREDIT. Added 3 September 2026, moved into this row on the
          same day: "it has to be in line with the 2026 Topcat Worktops Ltd all
          rights reserved and the sitemap privacy terms, not further down."

          So it is the MIDDLE child of `.foot-bar`, which is a
          `justify-content: space-between` row — copyright left, credit centre,
          legal links right. It is inside the bar rather than below it, which is
          what puts it on the rule's own line.

          ⛔ THE HEART IS AN INLINE SVG, NOT THE ❤ EMOJI. An emoji renders in
          the platform's own colour and shape, red on almost every device, so it
          could never be gold. Drawn here it takes `--gold` like every other
          accent on the page and is identical everywhere.

          `aria-label` carries the sentence in words and the heart is
          `aria-hidden`, so a screen reader says "Designed with love by Apex
          Growth" rather than announcing a path.

          Not a link yet, as asked. Wrapping it in an <a> is the whole change.
        */}
