...
(function(){
  ...
  const HOLDS=[...document.querySelectorAll('.cine-plates .plate')].map(el=>({
    el, t:+el.dataset.t, src:el.dataset.src, dwellVh:130, o:-1, wired:false
  }));
  if(HOLDS.length)HOLDS[HOLDS.length-1].dwellVh=182;   // the existing dead scroll, now plated
  let SEGS=[], trackPx=0;
  ...
  function plates(t){
    ...
    for(const H of HOLDS){
      ...
      const f=off*FPS;
      const k=clamp((f-0.5)/2.5);
      const o=H.wired?+(1-k*k*(3-2*k)).toFixed(3):0;
  ...
  if(skipEl)skipEl.addEventListener('click',()=>{
    ...
    seek(); ink(fEnd); veil(fEnd); story(want); plates(want); chrome(fEnd); grade();
  });
  ...
  function tick(){
    ...
    plates(want);     /* his generation stills, over the frames they were generated for */
    chrome(film);
    grade();
  ...
  addEventListener('load',()=>{
    ...
    const w=()=>{ wirePlate(HOLDS[0]); plates(want); };
