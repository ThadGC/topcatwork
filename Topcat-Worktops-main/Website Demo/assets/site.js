...
(function(){
  ...
  /* ⭐⭐⭐ **60fps SINCE D425 — THE FILE HAS THE FRAMES NOW, SO THE SCRUB CAN BE CONTINUOUS.**
     ...
     ⛔⛔⛔ **`SRCFPS` IS 12 AND IT IS NOT THE FILM'S RATE — IT IS THE TRACKING GRID'S.** The
     slab-reveal tables (D354 desktop, D360 phone, D368 tablet) were measured off a frame dump of
     the 12fps film: `.textanim-2026-08-24/track_edge.py` line 8 says so in its own words, *"frame
     number = film time * 12"*, and every row it prints is stamped `t = f/12`. So a row index is
     `mediaTime * 12`, WHATEVER rate the file is encoded at — the tables describe the picture's
     TIMELINE, which has never changed.
     ⛔⛔ **D416 TOOK THIS CONSTANT FROM 12 TO 24 WITH THE ENCODE AND THE TABLES WERE NOT RE-BASED,
     WHICH SILENTLY TURNED THE SLAB REVEAL OFF.** Every lookup came out at twice its row, ran past
     the end of the table, and `done` went true — so the clip came OFF and the copy sat ON TOP of
     the slab instead of behind it. Client: *"this text is supposed to be behind this slab… I don't
     understand why that text is now in front of the slab."* ⚠️ Nothing threw and nothing measured
     wrong: an out-of-range reveal looks exactly like a section with no reveal.
     ⭐ **THE LESSON, WRITTEN WHERE IT WILL BE READ:** the file's frame rate and the tables' grid
     are two different numbers and they must never be spelled with the same constant again. Change
     the encode freely; `SRCFPS` only changes if the tables are re-tracked. */
  const FPS=60, DUR=44.25;                // the file's own rate and length, and the fallback
  const SRCFPS=12;                        // the TRACKING grid of the reveal tables — never the file's rate
  ...
  function fail(){
    ...
    raf=null;
    /* ⚠️ D441: whichever route got us here, the watchdog has done its job — a live timer would
       only fire fail() a second time into a page that has already taken the static hero. */
  ...
  /* ⭐⭐⭐⭐ THE FILM THAT NEVER ARRIVES (D441) — a frozen poster while the story marched on.
     ...
     mobile video?"*
     ⭐⭐ IT IS THE RIGHT FILM AND THE RIGHT CODE — verified in the iOS Simulator on this same URL,
     where the phone cut scrubs correctly. What his phone had that the Simulator did not is a real
     network: `topcat-intro-608.mp4` is 7.7 MB, and he was recording DURING A PHONE CALL (the green
     pill is in his own status bar), which on a non-VoLTE connection starves data outright.
     ⛔⛔ AND THERE WAS NO GATE FOR THAT. `fail()` was wired to `error` only — but a film that is
     merely SLOW never errors, it just never comes. So `cine-on` stayed up, the scroll kept driving
     the story lines on schedule, and the picture underneath stayed on frame 0. **That combination —
     text advancing over a still — is what reads as broken**, and it is worse than having no film.
     ⭐ THE WATCHDOG USES THE DEGRADATION PATH THAT ALREADY EXISTS. `fail()` is the tested reduced-
     motion/decode-error route: it drops `cine-on`, releases the 800vh scroll and leaves the normal
     static hero. Reusing it means a starved connection lands somewhere already designed, not in a
     new state nobody has seen.
     ⚠️ IT ARMS ONLY WHEN THE FILM IS ACTUALLY WANTED — at the first scroll into the sequence, not
     at page load. A visitor who never scrolls must never trip it, and a slow first paint on a fast
  ...
  /* ⛔⛔⛔ **THE QUEUE MUST NOT TRUST `seeking` FOREVER — 25 Aug 2026 (D431). THIS FROZE THE FILM
     ON REAL iOS AND NOWHERE ELSE.** Client, with a screen recording: *"the video is no longer
     playing on mobile, so it's not moving."*
     ⭐⭐⭐ **CAUGHT IN THE iOS SIMULATOR WITH AN ON-PAGE READOUT, WHICH IS THE ONLY PLACE IT SHOWS.**
     The numbers were unambiguous: `tgt0.247 eas0.247 want13.86` — the engine correctly wanted frame
     831 — beside `sk0 seeking1 pend1 raf0`. **Safari had left `seeking` true on the very first seek
     and never fired `seeked`**, so `pending` was set, nothing drained it, and every seek for the
     rest of the session was refused by the first line of this function. One hung seek deadlocked
     the whole film. ⚠️ Desktop Chrome always fires `seeked`, so this cannot be reproduced there —
     three rounds of measuring it in a desktop harness would never have found it.
     ⭐ **SO THE GATE IS NOW TIME-BOUNDED.** A seek genuinely in flight is still respected — that is
     D310's rule and it is still right, a decoder handed a new time mid-seek drops the first one —
     but after `SEEK_STALL` the in-flight seek is treated as lost and the newest frame is issued
     anyway. Dropping a seek we no longer want is exactly what should happen.
     ⭐⭐ **AND THE FIRST STALL KICKS THE DECODER.** A muted `playsinline` video that has never
     decoded a frame is the case Safari hangs on; one `play()`/`pause()` pair puts it in a state
     where seeks resolve. It runs AT MOST ONCE and only after a stall, so no browser that works
     ever executes it. ⚠️ `muted` and `playsinline` are both already on the element — without them
     iOS refuses a programmatic `play()` outright. */
  const SEEK_STALL=140;
  /* ⭐⭐⭐ **THE DECODER IS WOKEN BEFORE THE FIRST SEEK, NOT AFTER IT HANGS — D431b.** Measured on
     the simulator after D431's watchdog: seeks were being ISSUED correctly (`want13.42`, `sk805`)
     and `currentTime` followed, but the PICTURE was still showing t≈3 — Safari was accepting the
     time and not presenting the frame, with `seeking` stuck true throughout.
     ⭐⭐ **A MUTED `playsinline` VIDEO THAT HAS NEVER DECODED A FRAME IS THE CASE SAFARI WILL NOT
     SCRUB.** One `play()` immediately followed by `pause()` puts the media element into the state
     where seeks resolve and present — it is the standard prerequisite for scrubbing video on iOS,
     and it is why every scroll-scrub implementation does it. ⛔ It has to happen on `loadeddata`,
     BEFORE the first seek; doing it only on a stall (D431) left the first seconds broken.
     ⚠️ Harmless everywhere else: the element is muted, it advances by one frame at t=0 and pauses
     inside the same task, and the opening plate is opaque over it at that moment (D333). ⚠️ It is
     also why `muted` and `playsinline` must stay on the element — without them iOS rejects the
     `play()` and the film never scrubs at all. */
  ...
  function seek(){
    ...
    if(vid.seeking){
      ...
      decoderKick();                 /* the in-flight seek is hung, not slow */
    }
    pending=false;
  ...
  /* ⚠️ A STALLED SEEK LEAVES NOTHING RUNNING TO RETRY IT: the chase stops itself once it has caught
     up, so without this the film would stay frozen until the next scroll event. One cheap timer,
     armed only while a seek is outstanding. */
